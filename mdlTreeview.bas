...
Public Function TvwCreateEx(ByVal hParent As Long) As Long
    ...
    hCont = CreateWindowEx(0&, "STATIC", "bTreeViewClass", WS_VISIBLE Or WS_CHILD, 0, 0, 250, 250, hParent, 0, App.hInstance, 0)
            ...
            If TVHT.hItem <> 0 Then
                For ax = 0 To iNodes - 1
                    If NodeX(ax).hItem = TVHT.hItem Then
                        ...
                        Form1.Label1.Caption = "Text =: " & NodeX(ax).Text & ", Key =: " & NodeX(ax).Key & ", Index =: " & NodeX(ax).Index
                    End If
                Next
            End If
